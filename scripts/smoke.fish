...
set link_flags -O0
set runtime_sources (find runtime -maxdepth 1 -type f -name '*.c' ! -name 'test_*.c' ! -name 'bytes.c' | sort)
...
or exit 1
mv bootstrap/compiler.ll bootstrap/stage1.ll
or exit 1

# 2. 链接新 stage1(-O0 加速冒烟;全量验证用 make bootstrap 的 -O2)
echo '== link stage1'
clang $llvm_flags $link_flags -o bootstrap/stage1 bootstrap/stage1.ll $runtime_sources -I runtime
or exit 1

# 3. 用新 stage1 的新流水线(DEBUG)编译目标测试
for source_file in $test_files
    echo "== $source_file"
    env DEBUG=true bootstrap/stage1 llvm "$source_file" -o tmp/smoke.ll
    or exit 1
    head -1 tmp/smoke.ll
end
rm -f tmp/smoke.ll
